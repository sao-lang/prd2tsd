...
| 维度 | 结论 | 证据 |
|---|---|---|
| 功能完整性 | ✅ | 输入护栏、PII/L3 脱敏、输出护栏、Failover 隔离、计费均覆盖同步与流式入口 |
| 功能间联通 | ✅ | Chat、知识问答、文档分析、SectionWriter 均经 `LLMGateway.stream_complete()` 自动继承修复 |
| 模块间联通 | ✅ | Gateway 对外方法签名不变；Provider、路由、熔断和调用方无需适配 |
| 可用性 | ⚠️ 环境受限 | 新增 7 测试、相关回归 82 测试、Ruff/mypy 通过；真实 Redis/PostgreSQL 未启动，E2E 未执行 |

### 发现并修复的问题

| 问题 | 严重程度 | 修复 |
|---|---|---|
| 流式 chunk 在完整输出护栏前已对外释放，跨 chunk 敏感内容可绕过检查 | 严重 | 按单次 Failover 尝试完整缓冲，后置护栏通过后才释放安全 chunk |
| 失败 Provider 的半截输出可能与 Failover 结果拼接 | 严重 | 每次 Provider 尝试使用独立缓冲，失败时整体丢弃 |
| 输出护栏后再还原可逆脱敏 token，会把敏感原文重新带回响应 | 严重 | 调整为先还原、再执行输出护栏和最终遮罩 |
| 在 `yield` 后记录成本，客户端提前停止消费会造成漏记 | 中等 | 在首个安全 chunk 释放前完成成本与 Token 统计 |

### 最终状态

流式与非流式调用已统一执行相同护栏语义。为保证输出安全，流式内容需要完整生成并检查后才开始释放；状态、心跳和任务进度事件仍可实时推送。
