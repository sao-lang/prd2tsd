...
- **时间：** 2026-09-03
- **发起人：** user（“继续 3.3–3.15 的链路分析，并将 3.1–3.15 详细版本单独出文档”）
- **修改文件：** `docs/interview-prep-chains-3.1-3.15-detailed.md`、`overview.md`
- **修改内容：** 基于当前源码新增并扩充 3.1–3.15 独立面试详解，串联统一入口、主编排、知识入图与检索、四层子图、人工审核、记忆、SSE、文档/URL 接入、认证授权和可观测性；在每条现有链路内补到实现粒度，例如各格式正文提取、段落/句子分块正则与字段、实体和 Claim 的 Prompt/JSON 解析、Neo4j/PGVector 写入、查询改写/RRF/反思/重排、Send 并发合并、JWT/RBAC、SSE 队列以及 Trace/回放数据裁剪。Gateway 章节进一步说明多 Provider 路由、熔断、Failover、原子限流预留与校正、两级语义缓存、预算降级、成本归因及 7 项护栏的执行顺序和实际接线边界。
- **复盘结果：** 修正原简版中易被误解的口径，包括当前入图未闭环关系写入、Planning 反馈未被节点消费、Generation 修订结果未进入最终组装、评测节点失败与真实 5 分无法区分、刷新 Token 丢失租户权限声明、DecisionRecorder 当前只记录阶段级裁剪状态，以及 RetryDecisionGuardrail 尚未接成自动重试循环等。
- **潜在风险：** 纯文档变更，无 API、数据库和配置影响；源码继续演进后需同步更新实现边界。
